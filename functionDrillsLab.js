...
// let nameGreeting = nameCheckResponse('Tony') 
...
let bestMovie = (string) => console.log(`${string} is the best movie ever!`)
...
function theEliminator(arr1, str){
  for(let i = 0; i < arr1.length; i++){
    if(arr1[i] === str){
      arr1.splice(i, 1) // ask about this  
    }
  }
...
let verifier = emailCheck('davidgmail.com')
...
function chocolateFrogBuyer (wallet){
  return wallet / 3
}

let totalFrogs = chocolateFrogBuyer(walletValue)
...
function chocolateFrogBuyer2(wallet){
  return Math.floor(wallet / 3)
  
}

let totalFrogs2 = chocolateFrogBuyer2(walletValue)
...
/*
  ...
  if it is sorted in ascending order, false if it is not. Create a variable, `arrayIsAscending` and set it equal to your function invoked. 
  Use the sample array to test this function.
*/

//CODE HERE

function arrAscend(arr2){
  for(i = 1; i < arr2.length; i++){
    let lastNumber = arr2[i - 1]
    if(arr2[i] < lastNumber){
      return false
    } 
  } 
  return true
...
let arrayIsAscending = arrAscend(sampleArray)
